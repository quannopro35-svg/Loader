...
-- Thay thế toàn bộ hàm AttackNoCoolDown bằng đoạn này
function AttackNoCoolDown()
    local hitList = {}
    local enemies = workspace:FindFirstChild("Enemies") and workspace.Enemies:GetChildren() or {}
    -- FindEnemiesInRange sẽ chèn vào hitList và trả về 1 part (head) nếu có
    local foundPart = FindEnemiesInRange(hitList, enemies)

    if (#hitList == 0) then
        return
    end

    -- Đảm bảo có tool được trang bị; nếu không thì cố equip SelectWeapon
    local equipped = GetEquippedTool()
    if not equipped then
        pcall(function() EquipTool(SelectWeapon) end)
        wait(0.12)
        equipped = GetEquippedTool()
        if not equipped then
            -- cố equip tool bất kỳ trong backpack nếu SelectWeapon không có
            for _, t in ipairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                if t:IsA("Tool") then
                    pcall(function() game.Players.LocalPlayer.Character.Humanoid:EquipTool(t) end)
                    equipped = GetEquippedTool()
    ...
    pcall(function()
        ...
        if #hitList > 0 then
            -- Lấy target part hợp lệ: ưu tiên HumanoidRootPart, fallback Head (hoặc phần trả về bởi FindEnemiesInRange)
            local first = hitList[1]
            local targetModel = first[1] -- model
            local targetPart = nil
            if targetModel and targetModel:IsA("Model") then
                targetPart = targetModel:FindFirstChild("HumanoidRootPart") or targetModel:FindFirstChild("Head")
            end
            if not targetPart then
                -- fallback dùng phần trả về từ FindEnemiesInRange (foundPart) nếu có
                targetPart = foundPart or (first[2] and first[2]) -- first[2] thường là head
            end

            -- Gửi lên server
            registerAttack:FireServer(1e-9)
            if targetPart then
                registerHit:FireServer(targetPart, hitList)
            else
                -- nếu không có part, vẫn gửi list (một số server chấp nhận)
                registerHit:FireServer(foundPart, hitList)
            end
        end
    end)
end
