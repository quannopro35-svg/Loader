...
local v15 = v14:CreateWindow({
    Title = " KING DEV [ Premium ]",-- thay tên bạn muốn đặt
    SubTitle = "By QUÂN",-- tên bạn
    TabWidth = 160,
    Theme = "Dark",--màu
    Acrylic = false,
...
local RigController = CombatFrameworkR.activeController

function AttackReal()
    if RigController and not RigController.attacking then
        RigController:attack()
    end
end

_G.Fast_Delay = 0.12 -- an toàn, đừng thấp hơn
        ...
        if _G.AutoLevel then
            pcall(function()
                ...
                --// === FARM QUÁI ===
                    ...
                    for _, mob in pairs(workspace.Enemies:GetChildren()) do
                        ...
                        and mob.Name == Ms then
                            ...
                            repeat
                                ...
                                -- ĐỨNG CAO HƠN MOB
                                Tween(mob.HumanoidRootPart.CFrame * CFrame.new(0,5,0))

                                -- HITBOX AN TOÀN (KHÔNG BỊ BLOCK)
                                mob.HumanoidRootPart.Size = Vector3.new(25,25,25)
                    ...
                    --// === TÌM SPAWN QUÁI ===
                    for _, spawnMob in pairs(workspace._WorldOrigin.EnemySpawns:GetChildren()) do
                        if string.find(spawnMob.Name, NameMon) then
                            if (LocalPlayer.Character.HumanoidRootPart.Position - spawnMob.Position).Magnitude >= 10 then
                                Tween(spawnMob.CFrame * Pos)
                            end
                        end
                    end
                end
    ...
    function getHighestPoint()
        ...
        for v728, v729 in pairs(game:GetService("Workspace").Map.MysticIsland:GetDescendants()) do
            if v729:IsA("MeshPart") then
                if (v729.MeshId == "rbxassetid://75697026305806") then --thay id ảnh vào
                    return v729;
                end
            end
        end
